...
def download_yt_audio(url):
    if not url:
        return (gr.update(value=None), gr.update(visible=False), gr.update(visible=False), gr.update(visible=False), gr.update(visible=False), gr.update(visible=False), gr.update(visible=False), gr.update(visible=False))
    try:
        ydl_opts = {
        ...
        # Export full audio to wav for preview
        full_path = 'temp_full.wav'
        audio.export(full_path, format="wav")
        duration_sec = duration_ms / 1000.0
        
        # Random start for initial 8s or full if short
        ...
        output_path = os.path.join("prompts", f"yt_{int(time.time())}.wav")
        excerpt.export(output_path, format="wav")
        
        # Initial segment preview
...
def preview_segment(start_sec, end_sec, full_path):
    ...
    try:
        ...
        end_ms = int(min(duration_sec, end_sec) * 1000) if 'duration_sec' in locals() else min(int(end_sec * 1000), duration_ms)
        if end_ms <= start_ms:
            return gr.update(value=None, visible=False)
        excerpt = audio[start_ms:end_ms]
        temp_seg = 'temp_segment.wav'
...
def trim_and_load(start_sec, end_sec, full_path):
    if not full_path or not os.path.exists(full_path):
...
with gr.Blocks(title="IndexTTS Demo") as demo:
    ...
    yt_button.click(download_yt_audio, 
                    inputs=[yt_url], 
                    outputs=[prompt_audio, full_audio_preview, start_time, end_time, segment_preview, trim_button, full_path_hidden, refine_row])

    start_time.change(preview_segment,
                        inputs=[start_time, end_time, full_path_hidden],
                        outputs=[segment_preview])

    end_time.change(preview_segment,
                        inputs=[start_time, end_time, full_path_hidden],
                        outputs=[segment_preview])

    trim_button.click(trim_and_load,
                      inputs=[start_time, end_time, full_path_hidden],
